...
public class FamilyThread extends Thread{
    ...
    public void run() {
        try {
            ...
            // run이 동시에 실행된 후에 openDoor메서드를 호출하기 전까지는 지연을 시키든 뭐든 일단 opendoor 실행직전까지 스레드가 접근한다는것
            ...
            // 결과를 보면 알다시피 그 대신 스레드에 접근하는 객체의 순서는 코드가 짜여진 순서가 아니라는 것이다. 다만 같은 메서드에서 서로 다른
            // 객체가 구동되며 프로그램이 꼬이는 일은 방지할 수 있다.
