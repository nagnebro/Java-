...
//    // 타입이 정해져 있어야된다.
//    // ->  제네릭은 아직 메모리에 올라가있는 상태가 아니기 때문에 static 멤버로 사용할 수 없다. static은 컴파일 시에 메모리에 올라가고 제네릭의 타입은
//           프로그램이 실행된 후에 올라가기 때문.
...
////        list[1] = new Sample();// 아래와 같이 생략 가능
////        // 근데 이렇게 저장하면 타입이 어떻게 되는 거지..?
//             ->   타입이 어떻게 저장된다기보다는 이제 Sample클래스를 이용할 떄 제네릭타입에 맞는 매개변수를 주면 그 클래스에서 지정해놓은
//                  멤버에 맞게 사용되는 것.
////
////        // 제네릭은 타입을 지정해준다. -> 그래서? 그 외의 타입이 들어오면 에러가 난다.
////        // 근데 Sample타입의 참조변수 배열 list가 있는데 그 안의 요소들이 Integer타입이라는 건가..? -> 맞음, String집어넣으면   String 타입일거고.
////        list[2] = new Sample<>();
////    }
//}